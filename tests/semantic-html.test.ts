...
describe("Semantic HTML Styling", () => {
	...
	describe("Form Elements", () => {
		...
		test("select elements should match input styling", async () => {
			const selectStyles = await page.evaluate(() => {
				...
				return {
					selectRadius: parseFloat(getComputedStyle(select).borderRadius),
		...
		test("images should be responsive", async () => {
			const imageStyles = await page.evaluate(() => {
				...
				return {
					...
					borderRadius: parseFloat(styles.borderRadius),
					fitsInContainer:
						rect.width <= (img.parentElement?.offsetWidth || Infinity),
				};
			});
	...
	describe("Additional Semantic Elements", () => {
		test("figure and figcaption should be styled", async () => {
			const figureStyles = await page.evaluate(() => {
				const figure = document.querySelector("figure");
				const figcaption = document.querySelector("figcaption");
				if (!figure || !figcaption) return null;
				const figureStyle = getComputedStyle(figure);
				const captionStyle = getComputedStyle(figcaption);
				return {
					figureMargin: figureStyle.margin,
					captionFontSize: parseFloat(captionStyle.fontSize),
					captionTextAlign: captionStyle.textAlign,
				};
			});

			if (figureStyles) {
				expect(figureStyles.captionTextAlign).toBe("center");
				expect(figureStyles.captionFontSize).toBeGreaterThan(0);
			}
		});

		test("definition lists should have proper hierarchy", async () => {
			const dlStyles = await page.evaluate(() => {
				...
				return {
					...
					ddMarginLeft: parseFloat(ddStyle.marginLeft),
				};
			});

			if (dlStyles) {
				expect(
					Number.parseInt(dlStyles.dtFontWeight, 10),
				).toBeGreaterThanOrEqual(600);
				expect(dlStyles.ddMarginLeft).toBeGreaterThan(0);
			}
		});

		test("abbr should have help cursor and underline", async () => {
			const abbrStyles = await page.evaluate(() => {
				const abbr = document.querySelector("abbr[title]");
				if (!abbr) return null;
				const styles = getComputedStyle(abbr);
				return {
					cursor: styles.cursor,
					textDecoration: styles.textDecoration,
				};
			...
			if (abbrStyles) {
				expect(abbrStyles.cursor).toBe("help");
				expect(abbrStyles.textDecoration).toContain("underline");
			}
		});

		test("sub and sup should have proper positioning", async () => {
			const scriptStyles = await page.evaluate(() => {
				...
				return {
					...
					supPosition: supStyle.position,
					subBottom: subStyle.bottom,
					supTop: supStyle.top,
				};
			});

			if (scriptStyles) {
				expect(scriptStyles.subPosition).toBe("relative");
