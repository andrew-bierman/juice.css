	...
	for (const viewport of viewports) {
		describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
			...
			test("body should have proper max-width and centering", async () => {
				...
				expect(bodyLayout.margin).toContain("auto"); // Centered
			...
			test("touch targets should be large enough on mobile", async () => {
				if (viewport.type === "mobile") {
					const touchTargets = await page.evaluate(() => {
						...
						return Array.from(buttons).map((el) => {
							return {
								...
								meetsMinimum: (el as HTMLElement).offsetHeight >= 44, // Apple's recommended 44px
							};
						});
					});
					...
					expect(percentage).toBeGreaterThan(70); // At least 70% should meet minimum
				}
			});
